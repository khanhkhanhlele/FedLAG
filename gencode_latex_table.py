        ...
        for alpha in alphas:
            ...
            for user_count in user_counts:
                ...
                if user_count != user_counts[-1]:
                    latex_code += "    \\hfill\n"
            latex_code += f"    \\caption{{The figures illustrate the performance of FL-LAG vs. various baselines on {dataset}. The evaluation is implemented on different numbers of users (from left to right, from top to bottom, the number of users is $U = {','.join(map(str, user_counts))}$, respectively). The sampling rate is set to ${alpha}$}}\n"
